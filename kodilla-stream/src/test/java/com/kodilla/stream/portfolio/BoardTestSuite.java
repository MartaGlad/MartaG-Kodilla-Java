...
public class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        Board project = prepareTestData();
        //When
        int sumDaysInProgress = project.getTaskLists().stream()
                .filter(tl -> tl.getName().equals("In progress"))
                .flatMap(tl -> tl.getTasks().stream())
        ...
        long numberTasksInProgress = project.getTaskLists().stream()
                .filter(tl -> tl.getName().equals("In progress"))
                .flatMap(tl -> tl.getTasks().stream())
                .count();

        double averageDaysInProgress = (double) sumDaysInProgress / numberTasksInProgress;

        //Then
        assertEquals(30, sumDaysInProgress);
        assertEquals(3, numberTasksInProgress);
    ...
    void testAddTaskListAverageWorkingOnTask2(){
        ...
        double averageDaysInProgress = project.getTaskLists().stream()
                .filter(tl -> tl.getName().equals("In progress"))
                .flatMap(tl -> tl.getTasks().stream())
                .mapToDouble(d ->  Period.between(d.getCreated(), LocalDate.now()).getDays())
                .average().getAsDouble();
